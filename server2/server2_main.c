...
int main(int argc, char *argv[])
{
    ...
    socklen_t addrlen;
    uint32_t childpid;         /* pid of child process */
    uint32_t conn_request_skt; /* passive socket */
...
#define LISTENQ 1024
    ...
    while (1)
    {
        ...
        {
            /* parent process */
            close(newSocketNumber); /* close connected socket */
        }
        else
        {
            /* child process */
            close(conn_request_skt);  /* close passive socket */
            service(newSocketNumber); /* serve client */
            exit(0);
        }
    }
...
void service(uint32_t newSocketNumber)
{
    ...
    while (read(newSocketNumber, &receivedCharacter, 1) > 0)
    {
        ...
        /*Handling wrong input from the client*/
        if (totalBytesRead == RECEIVING_BUFFER_SIZE)
        {
        ...
        if ((receivingBuffer[0] == 'G' || receivingBuffer[0] == 'g') && (receivingBuffer[1] == 'E' || receivingBuffer[1] == 'e') && (receivingBuffer[2] == 'T' || receivingBuffer[0] == 't') && receivingBuffer[counter - 2] == '\r')
        {
            ...
            {
                fileName[i - 4] = receivingBuffer[i]; //for the professor test
                i++;
            }
            printf("The file name is %s\n", fileName);
            printf("The file name length %ld\n", strlen(fileName));
